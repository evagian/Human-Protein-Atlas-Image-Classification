    ...
    with load_data(filepath='./dataset/transformed_data/protein_image_patches_pat_size_300_bat_size_32_1001_10000.npy') as data:
        with load_data(filepath='./dataset/transformed_data/protein_image_classes_pat_size_300_bat_size_32_1001_10000.npy') as datay:
            ...
            train_labels = pd.read_csv(
                './dataset/kaggle_protein_classes_augmented_one_hot.csv')
...
def denoiser_test(denoiser):
    ...
    test_filesy = train_labels.loc[101:200]
    test_datay = test_filesy.iloc[:, 4:]

    #test_datay = test_filesy

    # get image id
    test_data_im_id = train_labels.loc[101:200, "Id"]

    # get each image channel as a greyscale image (second argument 0 in imread)

    test_files = []
    for eval_im_id in test_data_im_id:
        test_files.append(glob('./dataset/train/{}_green.png'.format(eval_im_id)))
    # data_files.sort()
